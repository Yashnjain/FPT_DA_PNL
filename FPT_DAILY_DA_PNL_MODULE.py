...
config = get_config('FPT_DAILY_PNL','FPT_DAILY')
print("emails", config['EMAIL_LIST'])

job_id=np.random.randint(1000000,9999999)
for handler in logging.root.handlers[:]:
    logging.root.removeHandler(handler)
database='POWERDB'
process_name='FPT_DAILY_DA_PNL_MODULE'
# log file location
log_file_location=os.getcwd() + '\\' + 'logs' + '\\' + f'{process_name}_log.txt'
...
# receiver_email_business = 'power@biourja.com,indiapowerit@biourja.com,Anil.Kumar@BioUrja.com'
# receiver_email = config['EMAIL_LIST']
#receiver_email_business = receiver_email = 'rohit.pawar@biourja.com,pritish.jain@biourja.com,vihar.vaghasiya@biourja.com'
# receiver_email_business = "nikhil.chouhan@biourja.com"
# receiver_email = "nikhil.chouhan@biourja.com"
...
def get_inventory(month_start):
    try:
        sql='''select fpt_run.BU_ISO,fpt_run.AUCTION_NAME,fpt_run.AUCTION_ROUND,fpt_run.FLOW_STARTDATE FLOW_MONTH,fpt_run.BU_HEDGE_TYPE,fpt_run.BU_TRADE_TYPE,
            ...
            fpt_fpd.SINK_PNODE,snk_bup.YES_PNODEID SINK_ID,fpt_fpd.BU_PEAK_TYPE,fpt_fpd.BID_MW_TRADER FTR_MW, fpt_fpd.BID_PRICE_MWH_TRADER FTR_COST,fpt_fpd.PA_MCP_MWH
...
if __name__ == "__main__":
    ...
    try:
        ...
        bu_alerts.bulog(process_name=f'{database}',database=f'{database}',status='Started',table_name= f'{database}.PQUANT.FPT_DAILY', row_count= rows, log=log_json, warehouse='QUANT_WH',process_owner='Rahul')
        ...
        MKT_HRCNT = pd.melt(market_times,id_vars=['ISO','DATETIME','MARKETDAY'],value_vars=['OFFPEAK','ONPEAK','WEPEAK','WDPEAK','24HR', 'OFF7X8'])
        MKT_HRCNT.columns = ['BU_ISO','DATETIME','MARKETDAY','BU_PEAK_TYPE','HRCNT']

        LMP_PRICES = pd.merge(PRICE_DTLS,market_times[['ISO','DATETIME','MARKETDAY','ONPEAK','OFFPEAK','WEPEAK','WDPEAK','24HR','OFF7X8']],on=['ISO','DATETIME'],how='left')
        ...
        LMP_PRICES['DA_OFF7X8'] = LMP_PRICES['DALMP']*LMP_PRICES['OFF7X8']

        LMP_DA_PRICES = LMP_PRICES.drop(['ONPEAK','OFFPEAK','WEPEAK','24HR','WDPEAK','DALMP','RTLMP','OFF7X8'],axis=1)
        LMP_DA_PRICES = LMP_DA_PRICES.rename(columns = {'DA_ON_PK':'ONPEAK','DA_OFF_PK':'OFFPEAK','DA_WE_PK':'WEPEAK','DA_WD_PK':'WDPEAK','DA_24HR':'24HR','DA_OFF7X8':'OFF7X8'})
        SRC_LMP_DA_PRICES = pd.melt(LMP_DA_PRICES,id_vars=['ISO','DATETIME','MARKETDAY','NODE'],value_vars=['OFFPEAK','ONPEAK','WEPEAK','WDPEAK','24HR','OFF7X8'])
        ...
        FPT_DAILY_EXISTStxt = '''select fpd.* from {}.PQUANT.FPT_DAILY fpd where fpd.FLOW_DATE = '{}' order by FPT_ID asc'''.format(database,Current_Date)
        ...
        if (FPT_DAILY_EXISTS.shape[0] > 0):
            conn = get_connection(role=f'OWNER_{database}',database=f'{database}',schema='PQUANT')
            cs=conn.cursor()
            cs.execute(f'''delete from {database}.PQUANT.FPT_DAILY where FLOW_DATE = \'''' + Current_Date+ '\'')
            conn.close()
        ...
        FPT_DAILY_INV_DTLS['MtM_PnL'] = np.where(FPT_DAILY_INV_DTLS['BU_TRADE_TYPE'] == 'BUY', (FPT_DAILY_INV_DTLS['PA_MCP_MWH']-FPT_DAILY_INV_DTLS['FTR_COST'])*FPT_DAILY_INV_DTLS['FTR_MW']*FPT_DAILY_INV_DTLS['HRCNT'], -1*(FPT_DAILY_INV_DTLS['PA_MCP_MWH']-FPT_DAILY_INV_DTLS['FTR_COST'])*FPT_DAILY_INV_DTLS['FTR_MW']*FPT_DAILY_INV_DTLS['HRCNT'])

        stats=FPT_DAILY_INV_DTLS.groupby(['BU_ISO','DAYNO', 'BU_TRADER']).agg({'DA_PNL':'sum','MTD_INVEST':'sum','MtM_PnL':'sum'}).reset_index()
        stats.DAYNO = stats.DAYNO.astype(str)
        stats.DA_PNL = stats.DA_PNL.astype(float)
        stats.MTD_INVEST = stats.MTD_INVEST.astype(float)
        pnl_by_part=stats.groupby(['BU_ISO','BU_TRADER']).agg({'DA_PNL':'sum','MTD_INVEST':'sum','MtM_PnL':'sum'}).reset_index()
        ...
        subject = '{} FTR DA_PNL MTD as of  {}'.format(process_name,datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        ...
        msgbody=msgbody+'<td style="font-weight: bold; text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;">NET_INVST_MTD</td>'
        msgbody=msgbody+'<td style="font-weight: bold; text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;">MTM_MTD</td>'
        msgbody=msgbody+'<td style="font-weight: bold; text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;">PNL_MTD</td>'

        daynos=list(pnl_by_dayno['DAYNO'].unique())
        daynos.sort(reverse=True)
        ...
        if stats['MTD_INVEST'].sum() > 0:
            msgbody=msgbody+'<td style="font-weight: bold;text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:solid grey;color:green">{:,.0f}</td>'.format(stats['MTD_INVEST'].sum())
        else:
            msgbody=msgbody+'<td style="font-weight: bold;text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:solid grey;color:red">{:,.0f}</td>'.format(stats['MTD_INVEST'].sum())
        
        if stats['MtM_PnL'].sum() > 0:
            msgbody=msgbody+'<td style="font-weight: bold;text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:solid grey;color:green">{:,.0f}</td>'.format(stats['MtM_PnL'].sum())
        else:
            msgbody=msgbody+'<td style="font-weight: bold;text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:solid grey;color:red">{:,.0f}</td>'.format(stats['MtM_PnL'].sum())

        if stats['DA_PNL'].sum()>0:
            msgbody=msgbody+'<td style="font-weight: bold;text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:green">{:,.0f}</td>'.format(stats['DA_PNL'].sum())
        else:
            msgbody=msgbody+'<td style="font-weight: bold;text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:red">{:,.0f}</td>'.format(stats['DA_PNL'].sum())
        ...
        for i, x in stats_trans.iterrows():
            ...
            if x['MTD_INVEST'] > 0:
                msgbody=msgbody+'<td style="font-weight: bold; text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:green">{:,.0f}</td>'.format(round(x['MTD_INVEST']))
            else:
                msgbody=msgbody+'<td style="font-weight: bold; text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:red">{:,.0f}</td>'.format(round(x['MTD_INVEST']))
            
            if x['MtM_PnL'] > 0:
                msgbody=msgbody+'<td style="font-weight: bold; text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:green">{:,.0f}</td>'.format(round(x['MtM_PnL']))
            else:
                msgbody=msgbody+'<td style="font-weight: bold; text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:red">{:,.0f}</td>'.format(round(x['MtM_PnL']))
            
            if x['DA_PNL']>0:
                msgbody=msgbody+'<td style="font-weight: bold;text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:green">{:,.0f}</td>'.format(x['DA_PNL'])
            else:
                msgbody=msgbody+'<td style="font-weight: bold;text-align:center;border-bottom: 1px solid grey; border-right:1px solid grey;color:red">{:,.0f}</td>'.format(x['DA_PNL'])
        ...
        log_json='[{"JOB_ID": "'+str(job_id)+'","CURRENT_DATETIME": "'+str(datetime.now())+'"}]'
        bu_alerts.bulog(process_name=f"{process_name}",database=f'{database}',status='Completed',table_name= f'{database}.PQUANT.FPT_DAILY', row_count=rows, log=log_json, warehouse='QUANT_WH',process_owner='Rahul')
        bu_alerts.send_mail(
            receiver_email = receiver_email,
            mail_subject=f'JOB SUCCESS - {process_name}',
        ...
        logging.exception(f'Exception caught during execution: {e}')
        log_json='[{"JOB_ID": "'+str(job_id)+'","CURRENT_DATETIME": "'+str(datetime.now())+'"}]'
        bu_alerts.bulog(process_name=f'{process_name}',database=f'{database}',status='Failed',table_name=f'{database}.PQUANT.FPT_DAILY', row_count=rows, log=log_json, warehouse='QUANT_WH',process_owner='Rahul') 
        bu_alerts.send_mail(
            receiver_email = receiver_email,
            mail_subject=f'JOB FAILED - {process_name}',
